...
export default function DartaChalaniPage() {
  ...
  useEffect(() => {
    ...
    filters,
    pagination.page,
    dartaPagination.page,
    chalaniPagination.page,
    recordType,
  ]);
          ...
          fetch(`${API_URL}/darta?${dartaParams.toString()}`, {
            ...
            if (!res.ok) {
              const error = await res
                .json()
                .catch(() => ({ error: 'Failed to fetch darta records' }));
              return {
                error: error.message || error.error || 'Failed to fetch darta records',
                dartas: [],
                pagination: { total: 0 },
              };
            }
        ...
        promises.push(
          fetch(`${API_URL}/patra-chalani?${chalaniParams.toString()}`, {
            ...
            if (!res.ok) {
              const error = await res
                .json()
                .catch(() => ({ error: 'Failed to fetch chalani records' }));
              return {
                error: error.message || error.error || 'Failed to fetch chalani records',
                patraChalanis: [],
                pagination: { total: 0 },
              };
            }
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">
              Digital Darta / Chalani
            </h1>
            <p className="mt-2 text-sm text-gray-500 max-w-2xl">
              Manage all incoming (Darta) and outgoing (Chalani) official correspondence in one
                ...
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M3 10h18M3 14h18m-9-4v8m-7-8v8m14-8v8M5 21l7-7 7 7"
                    />
                  </svg>
                </button>
              </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-8 p-6">
              ...
              <div>
                ...
                {dartas.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table
                      className={`min-w-full divide-y divide-gray-200 ${viewMode === 'kitab' ? 'border-collapse border border-gray-300' : ''}`}
                    >
                      <thead className={viewMode === 'kitab' ? 'bg-gray-100' : 'bg-gray-50'}>
                        <tr>
                          <th
                            className={`px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${viewMode === 'kitab' ? 'border border-gray-300' : ''}`}
                          >
                            Darta No.
                          </th>
                          <th
                          ...
                          <th
                            className={`px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider ${viewMode === 'kitab' ? 'border border-gray-300' : ''}`}
                          >
                            Actions
                          </th>
                        </tr>
                      ...
                      <tbody
                        className={`bg-white divide-y divide-gray-200 ${viewMode === 'kitab' ? 'text-sm' : ''}`}
                      >
                        {dartas.map((record) => (
                          <tr
                            key={record.id}
                            className={`hover:bg-gray-50 transition-colors group ${viewMode === 'kitab' ? 'border-b border-gray-300' : ''}`}
                          >
                            <td
                              className={`px-6 py-4 whitespace-nowrap ${viewMode === 'kitab' ? 'border border-gray-300 py-2' : ''}`}
                            >
                            ...
                            <td
                              className={`px-6 py-4 ${viewMode === 'kitab' ? 'border border-gray-300 py-2' : ''}`}
                            >
                            ...
                            <td
                              className={`px-6 py-4 whitespace-nowrap text-right text-sm font-medium ${viewMode === 'kitab' ? 'border border-gray-300 py-2' : ''}`}
                            >
                              <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                  onClick={() =>
                                    setSelectedRecord({ type: 'darta', id: record.id })
                                  }
                                ...
                                <button
                                  onClick={() => handleDelete('darta', record.id, record.title)}
                ...
                {dartaPagination.total > dartaPagination.limit && (
                  <div className="mt-4 flex items-center justify-between">
                    <p className="text-sm text-gray-700">
                      Showing {(dartaPagination.page - 1) * dartaPagination.limit + 1} to{' '}
                      {Math.min(
                        dartaPagination.page * dartaPagination.limit,
                        dartaPagination.total
                      )}{' '}
                      of {dartaPagination.total}
                    </p>
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          setDartaPagination((prev) => ({ ...prev, page: prev.page - 1 }))
                        }
                      ...
                      <button
                        ...
                        disabled={
                          dartaPagination.page * dartaPagination.limit >= dartaPagination.total
                        }
                        className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Next
              ...
              <div>
                      ...
                      >
                        {patraChalanis.map((record) => (
                          ...
                          >
                            ...
                            >
                              <div className="text-sm text-gray-600">
                                {(record as PatraChalani).receiverName || '-'}
                              </div>
                            </td>
                            <td
                              className={`px-6 py-4 whitespace-nowrap ${viewMode === 'kitab' ? 'border border-gray-300 py-2' : ''}`}
                            ...
                            >
                              <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                  onClick={() =>
                                    setSelectedRecord({ type: 'chalani', id: record.id })
                                  }
                                  className="text-indigo-600 hover:text-indigo-900"
                                >
                                  View
                                ...
                                <button
                                  onClick={() => handleDelete('chalani', record.id, record.subject)}
                                  className="text-red-600 hover:text-red-900"
                                >
                                  <svg
                                    className="w-4 h-4"
                ...
                {chalaniPagination.total > chalaniPagination.limit && (
                  <div className="mt-4 flex items-center justify-between">
                    <p className="text-sm text-gray-700">
                      Showing {(chalaniPagination.page - 1) * chalaniPagination.limit + 1} to{' '}
                      {Math.min(
                        chalaniPagination.page * chalaniPagination.limit,
                        chalaniPagination.total
                      )}{' '}
                      of {chalaniPagination.total}
                    </p>
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          setChalaniPagination((prev) => ({ ...prev, page: prev.page - 1 }))
                        }
            ...
            <div className="p-12 text-center bg-gray-50/50">
              ...
              <p className="text-gray-500 mb-6 max-w-md mx-auto">
                No documents match your current filters. Try adjusting your search or create a new
                record.
              </p>
                  ...
                  {allRecords.map((record) => (
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center">
                          <div
                            className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold text-white mr-3 
                          ...
                          <div>
                            ...
                            {record.recordType === 'chalani' &&
                              (record as PatraChalani).patraNumber && (
                            ...
                            <>
                              {(record as PatraChalani).type === 'INCOMING' ? (
                                <span className="text-gray-900 font-medium">
                                  From: {(record as PatraChalani).senderName || '-'}
                                </span>
                              ) : (
                                <span className="text-gray-900 font-medium">
                                  To: {(record as PatraChalani).receiverName || '-'}
                                </span>
                              )}
                            </>
                          )}
                      ...
                      <td
                        className={`px-6 py-4 whitespace-nowrap text-sm text-gray-500 ${viewMode === 'kitab' ? 'border border-gray-300 py-2' : ''}`}
                      >
                      ...
                      >
                        <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() =>
                              setSelectedRecord({ type: record.recordType, id: record.id })
                            }
                            className="text-indigo-600 hover:text-indigo-900"
                            title="View Details"
                          >
