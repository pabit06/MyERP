...
export default function EquityPage() {
  ...
  const fetchAccounts = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${API_URL}/accounting/accounts?type=equity`, {
  ...
  const fetchAccountBalances = async (accountsList: Account[]) => {
    if (!token) return;
  ...
  const buildTree = useCallback(
    (accountsList: Account[], expanded: Set<string>, balances: Record<string, number>) => {
      // Filter to only include equity accounts (double-check on frontend)
      const equityAccounts = accountsList.filter((acc) => acc.type.toLowerCase() === 'equity');

      const accountMap = new Map<string, AccountTreeNode>();
      const rootNodes: AccountTreeNode[] = [];

      // First pass: create all nodes (only for equity accounts)
      ...
      // Second pass: build tree structure (only link equity accounts)
      equityAccounts.forEach((account) => {
        const node = accountMap.get(account.id)!;
        if (account.parentId && accountMap.has(account.parentId)) {
          const parent = accountMap.get(account.parentId)!;
          // Only add if parent is also a equity
          if (parent.type.toLowerCase() === 'equity') {
            if (!parent.children) parent.children = [];
            parent.children.push(node);
          } else {
            // Parent is not a equity, so this becomes a root node
            rootNodes.push(node);
          }
        } else {
          rootNodes.push(node);
      ...
      <ProtectedRoute requiredModule="cbs">
        <div className="p-6">
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            <p className="text-amber-800">CBS module is not enabled for your subscription</p>
          </div>
        </div>
      </ProtectedRoute>
    );
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Chart of Accounts - Equity Accounts (इक्विटी खाताहरू)
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 flex items-center space-x-2 text-sm"
            >
        ...
        {stats && !isLoading && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200 rounded-xl p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-amber-700">Total Balance</p>
                  ...
                  <p className="text-xs text-amber-600 mt-1">कुल शेष</p>
                </div>
                <DollarSign className="h-10 w-10 text-amber-600 opacity-50" />
              </div>
            </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-6 shadow-sm">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-amber-900">Error Loading Accounts</h3>
              ...
              <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-t-2 border-gray-200">
                <div className="flex flex-col md:flex-row md:justify-between md:items-center space-y-2 md:space-y-0">
                  <div className="flex items-center space-x-6 text-sm">
                    ...
                    {searchQuery && (
                      <div>
                        <span className="text-gray-600">Filteamber:</span>
