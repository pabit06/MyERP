...
export default function LiabilitiesPage() {
  ...
  const fetchAccounts = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${API_URL}/accounting/accounts?type=liability`, {
  ...
  const buildTree = useCallback(
    (accountsList: Account[], expanded: Set<string>, balances: Record<string, number>) => {
      // Filter to only include liability accounts (double-check on frontend)
      const liabilityAccounts = accountsList.filter(
        (acc) => acc.type.toLowerCase() === 'liability'
      );

      const accountMap = new Map<string, AccountTreeNode>();
      const rootNodes: AccountTreeNode[] = [];

      // First pass: create all nodes (only for liability accounts)
      ...
      // Second pass: build tree structure (only link liability accounts)
      liabilityAccounts.forEach((account) => {
        const node = accountMap.get(account.id)!;
        if (account.parentId && accountMap.has(account.parentId)) {
          const parent = accountMap.get(account.parentId)!;
          // Only add if parent is also a liability
          if (parent.type.toLowerCase() === 'liability') {
            if (!parent.children) parent.children = [];
            parent.children.push(node);
          } else {
            // Parent is not a liability, so this becomes a root node
            rootNodes.push(node);
          }
        } else {
          rootNodes.push(node);
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Chart of Accounts - Liability Accounts (दायित्व खाताहरू)
              </p>
            </div>
          </div>
        ...
        {stats && !isLoading && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-gradient-to-br from-red-50 to-red-100 border border-red-200 rounded-xl p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-red-700">Total Balance</p>
                  ...
                  <p className="text-xs text-red-600 mt-1">कुल शेष</p>
                </div>
                <DollarSign className="h-10 w-10 text-red-600 opacity-50" />
              </div>
            </div>
